...
{     	
    ...
    ofstream f1out(string("torque_T_" +T_str+"_h_"+hmag_str + "_minusK_" + k_str + "_G_"
                    +g_str+".dat").c_str(),ios_base::app); 
    ofstream fout(string("EnergyMag_T_" +T_str+"_h_" + hmag_str + "_minusK_" + k_str + "_G_"
                    +g_str+".dat").c_str(),ios_base::app); 
  ...
  //  ofstream f2out("acc_rate.dat");
    f1out << "theta \t taux \t tauy \t  tauz "
          <<" \t tau_perp "<<endl;
    fout << "energy \t mx \t mx-error \t my \t my-error \t mz \t mz-error "
          <<endl;
...
//f2out << i<< '\t'<< beta<< '\t'  << energy << '\t' << double(moves_accepted)/(no_of_sites) << endl;
...
// https://journals-aps-org.proxy.library.cornell.edu/prb/pdf/10.1103/PhysRevB.24.1391
        ...
        f1out << setw(12)<< taux/(no_of_sites*N_mc)
              ...
              << setw(12) << (2*taux+2*tauy-tauz)/(no_of_sites*N_mc*3.0)<<endl;
// printing tau or torque to file 
